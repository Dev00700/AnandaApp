        ...
        success: function (response) {
            ...
            if (response != null) {
                ...
                for (var i = 0; i < response.masterdata.length; i++) {
                    ...
                    str += `<td> <div class="datepicker date input-group calenderwidth">
                    ...
                    str += "<td><input type='text'  id='openingstock_" + i + "' class = 'form-control form-control-sm Negativedecimalonlytwodigit'/> </td>";
                    str += "<td><input type='text'  id='totalreceipt_" + i + "' class = 'form-control form-control-sm Negativedecimalonlytwodigit'/> </td>";
                    str += "<td><input type='text'  id='totalissueqty_" + i + "' class = 'form-control form-control-sm Negativedecimalonlytwodigit'/> </td>";
                    str += "<td><input type='text'  id='closingstock_" + i + "' class = 'form-control form-control-sm Negativedecimalonlytwodigit'/> </td>";
...
$("#save").on("click", function () {
    ...
    let totalrecptqty = $("#TotalReceipt").val();
    let totalissueqty = $("#TotalIssueQuantities").val();
    let closingstock = $("#ClosingStock").val();
    let isActive = $("#IsActive").is(":checked");
    let ClosingBalanceGuid = $("#ClosingBalanceGuid").val();
    ...
    let req = JSON.stringify({
        ...
        TotalReceipt: totalrecptqty,
        TotalIssueQuantities: totalissueqty,
        ClosingStock: closingstock,
        IsActive: isActive,
        ClosingBalanceGuid: ClosingBalanceGuid
    });

    $.ajax({
        url: "/ClosingBalance/Save",
        type: "POST",
        contentType: "application/json",
        data: req,
        success: function (response) {
...
$("#savelist").on("click", function () {
    ...
    $("#closingbalancevaluebody tr").each(function () {
        ...
        if (PlantMaterialId && Fromdate && Todate && openingstock && totalreceipt && totalissueqty && closingstock) { // Only add valid rows
            records.push({
...
function loadTable(page, focusedColumn, focusedValue, rowsPerPage) {
    ...
    var filters = {
        ...
        MeterialCode: $('input[data-column="MeterialCode"]').val(),
        MeterialName: $('input[data-column="MeterialName"]').val(),
        FromDate: $('input[data-column="FromDate"]').val(),
        ToDate: $('input[data-column="ToDate"]').val(),
        OpeningStock: $('input[data-column="OpeningStock"]').val(),
        ClosingStock: $('input[data-column="ClosingStock"]').val(),
    };

    $.ajax({
        url: '/ClosingBalance/GetPagedUser',
        data: {
            page: page,
            rowperpage: rowsPerPage,
            PlantCode: filters.PlantCode,
            PlantName: filters.PlantName,
            MeterialCode: filters.MeterialCode,
            MeterialName: filters.MeterialName,
            FromDate: filters.FromDate,
            ToDate: filters.ToDate,
            OpeningStock: filters.OpeningStock,
            ClosingStock: filters.ClosingStock,
        },
        success: function (data) {

            $('#closingbalanceTableContainer').html(data);
            ...
            $('input[data-column="MeterialCode"]').val(filters.MeterialCode);
            $('input[data-column="MeterialName"]').val(filters.MeterialName);
            $('input[data-column="FromDate"]').val(filters.FromDate);
            $('input[data-column="ToDate"]').val(filters.ToDate);
            $('input[data-column="OpeningStock"]').val(filters.OpeningStock);
            $('input[data-column="ClosingStock"]').val(filters.ClosingStock);
